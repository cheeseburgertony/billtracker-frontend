...
const PopupAddBill = memo(
  forwardRef(
    (props: { detail?: BillDetailData; onReload?: () => void }, ref) => {
      const { detail = {} as BillDetailData, onReload } = props;

      const [show, setShow] = useState(false);
      const [payType, setPayType] = useState<"expense" | "income">("expense");
      const [date, setDate] = useState<Date | string>(new Date());
      const [amount, setAmount] = useState("");
      const [expense, setExpense] = useState<LabelItem[]>();
      const [income, setIncome] = useState<LabelItem[]>();
      const [currentType, setCurrentType] = useState<LabelItem>(
        {} as LabelItem
      );
      const [remark, setRemark] = useState("");
      const [showRemark, setShowRemark] = useState(false);

      const dateRef = useRef<{ show: () => void }>(null);
      ...
      useEffect(() => {
        if (detail.id) {
          ...
          setDate(dayjs(Number(detail.date)).toDate()); // 转换成原生Date对象
        }
      }, [detail]);
      ...
      const changePayType = (type: "expense" | "income") => {
        ...
        setCurrentType(
          type === "expense"
            ? (expense?.[0] as LabelItem)
            : (income?.[0] as LabelItem)
        );
      };
      ...
      const addBill = async () => {
        ...
        // 参数准备
        const params: AddBillParams & { id?: number } = {
          amount: Number(amount).toFixed(2),
          type_id: currentType.id as number,
          type_name: currentType.name,
          date: String(dayjs(date).unix() * 1000),
        ...
        if (detail.id) {
          params.id = detail.id;
          // 发送请求进行修改
          await postBillUpdateAPI(params as EditBillParams);
          Toast.show("修改成功");
        } else {
          // 发送请求进行添加
          ...
          setAmount("");
          setPayType("expense");
          setCurrentType(expense?.[0] as LabelItem);
          setDate(new Date());
          setRemark("");
          Toast.show("添加成功");
        }
      ...
      // 数字键盘输入
      const onKeyClick = (value: string | undefined) => {
        if (value === "delete") {
          ...
          <div className={s.addWrap}>
            <div className={s.filter}>
              <div className={s.type}>
                <span
                  className={classNames([s.expense], {
            ...
            <div className={s.money}>
              ...
              <span className={classNames(s.amount, s.animation)}>
                {amount}
              </span>
            </div>
                  ...
                  >
                    ...
                    >
                      <CustomIcon
                        ...
                        type={
                          typeMap[item.id.toString() as keyof typeof typeMap]
                            ?.icon
                        }
                      />
                    </span>
            ...
            <PopupDate
              ref={dateRef}
              onSelect={(value: Date | string) => setDate(value)}
            />
          </div>
        </Popup>
      );
