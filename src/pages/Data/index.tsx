...
import { getBillDataAPI } from "@/apis/bill";
import { TotalDatum } from "@/apis/types";

const MyIcon = Icon.createFromIconfont(
  "//at.alicdn.com/t/c/font_4668023_je4xn29sh.js"
);
...
const Data = memo(() => {
  ...
  useEffect(() => {
    const getBillData = async () => {
      ...
      const _incomeData = data.total_data
  ...
  // 绘制饼图方法
  const setPieChart = (data: TotalDatum[]) => {
    // 避免直接访问 window.echarts，使用类型断言
    if ((window as any).echarts) {
      // 初始化饼图,返回实例
      proportionChart = (window as any).echarts.init(
        document.querySelector("#proportion")
      );
  ...
  return (
    <div className={s.data}>
      <div className={s.total}>
        <div
          className={s.time}
          onClick={() => monthRef.current && monthRef.current.show()}
        >
      ...
      <PopupDate
        ref={monthRef}
        mode="month"
        onSelect={(month) => setCurrentMonth(month as string)}
      />
      {/* 排行区域 */}
            ...
            <div className={s.item} key={item.type_id}>
              <div className={s.left}>
                  ...
                  >
                    <CustomIcon
                      type={
                        item.type_id
                          ? typeMap[
                              item.type_id.toString() as keyof typeof typeMap
                            ].icon
                          : typeMap["1"].icon
                      }
                    />
                ...
                <div className={s.progress}>
                  ¥{Number(item.number).toFixed(2) || 0.0}
                </div>
              </div>
