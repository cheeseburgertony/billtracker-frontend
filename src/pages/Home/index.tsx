...
import { getBillListAPI } from "@/apis/bill";
import PopupType from "@/components/PopupType";
import PopupDate from "@/components/PopupDate";
import CustomIcon from "@/components/CustomIcon";
import PopupAddBill from "@/components/PopUpAddBill";
import Empty from "@/components/Empty";
import type { BillItemData, LabelItem } from "@/apis/types";

const Home = memo(() => {
  const [list, setList] = useState<BillItemData[]>([]);
  ...
  // 选择类型后触发(子组件传递回来调用)
  const onSelectType = (item: LabelItem) => {
      ...
      <div className={s.header}>
        ...
        <div className={s.typeWrap}>
          <div className={s.left}>
            <span className={s.title} onClick={showPopupType}>
              {currentType.name || "全部类型"}
            </span>
          </div>
          <div className={s.right}>
            <span className={s.time} onClick={showPopupDate}>
              {currentTime}
            </span>
          </div>
        </div>
